...
4. **SSL Bus Comp** — TCN-emulated SSL-style bus compressor.
...
axon/
  export/
    composite.py          ← compose nablafx-export bundles into Axon staging
...
scripts/
  ...
  export_axon.py             ← compose trained checkpoints into a staging dir
  build_axon_mac.sh          ← Mac one-shot: stage + build .clap
  install_axon_mac.sh        ← Mac one-shot: build + install to ~/Library/Audio/Plug-Ins/CLAP/
weights/
  auto_eq_refs/           ← per-class long-term spectrum references (.npz)
  axon_bundle/            ← shipped per-stage bundles (model.onnx + plugin_meta.json)
docs/
...
# Restart your DAW; load "Axon" from the CLAP plugin list.
```
...
shipping; saturator is shipping; SSL bus comp is shipping.
